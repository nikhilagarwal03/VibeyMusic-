...
import { fetchFromJioSaavn } from "#common/helpers";
...
import type {
  SongModel,
  SongSuggestionAPIResponseModel,
...
export class GetSongSuggestionsUseCase implements IUseCase<
...
> {
  ...
  async execute({ songId, limit }: GetSongSuggestionsArgs) {
    ...
    const { data, ok } = await fetchFromJioSaavn<
      z.infer<typeof SongSuggestionAPIResponseModel>
    >({
