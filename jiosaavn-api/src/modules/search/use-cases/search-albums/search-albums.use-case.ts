...
export class SearchAlbumsUseCase implements IUseCase<
  SearchAlbumsArgs,
...
> {
  ...
  async execute({
    ...
    limit,
    page,
  }: SearchAlbumsArgs): Promise<z.infer<typeof SearchAlbumModel>> {
    const { data } = await fetchFromJioSaavn<
      z.infer<typeof SearchAlbumAPIResponseModel>
    >({
