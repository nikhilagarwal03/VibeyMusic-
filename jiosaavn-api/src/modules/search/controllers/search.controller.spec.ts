...
describe("SearchController", () => {
  ...
  it("search everything", async () => {
    const response = await searchController.controller.request(
      "/search?query=blurryface+twenty+one+pilots",
    );
    ...
    expect(() =>
      SearchModel.omit({ topQuery: true }).parse(data),
    ).not.toThrow();
  });

  it("search songs", async () => {
    const response = await searchController.controller.request(
      "/search/songs?query=believer",
    );
  ...
  it("search albums", async () => {
    const response = await searchController.controller.request(
      "/search/albums?query=blurryface+twenty+one+pilots",
    );

    const { data } = (await response.json()) as {
      success: boolean;
      data: z.infer<typeof SearchAlbumModel>;
    };
  ...
  it("search artists", async () => {
    const response = await searchController.controller.request(
      "/search/artists?query=adele",
    );

    const { data } = (await response.json()) as {
      success: boolean;
      data: z.infer<typeof SearchArtistModel>;
    };
  ...
  it("search playlists", async () => {
    const response = await searchController.controller.request(
      "/search/playlists?query=indie",
    );

    const { data } = (await response.json()) as {
      success: boolean;
      data: z.infer<typeof SearchPlaylistModel>;
    };
